...
const logFilePath = './log/parseSortSiteDeepCompatibilityReport.log';
 ...
 */
function parseCompatibilityExpando($expandoTbody) {
  const result = [];
  let currentDetails = [];
...
/**
 * Parses the full comptibility report HTML into structured data.
 * @param {string} html
...
function parseDeepCompatibilityReport(html) {
  ...
  try {
      ...
      $mainTbody.find('tr[id]').each(function () {
        try {
          ...
          if ($expandoTbody.length > 0) {
            const expandoResult = parseCompatibilityExpando($expandoTbody);
    ...
    logMessage(`Error parsing Compatibilty report HTML: ${outerError.message}`);
  }
  ...
  return [
    {
        header: "Deep Compatibilty Report",
        data: dataArray,
        reportType: "Deep"
    }
];
...
module.exports = parseDeepCompatibilityReport;
