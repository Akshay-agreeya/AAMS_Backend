...
const logFilePath = './log/parseSortSiteDeepErrorReport.log';
 ...
 */
function parseErrorExpando($expandoTbody) {
  const result = [];
  let currentDetails = [];
...
function parseDeepErrorReport(html) {
  ...
  try {
      ...
      $mainTbody.find('tr[id]').each(function () {
        try {
          ...
          if ($expandoTbody.length > 0) {
            const expandoResult = parseErrorExpando($expandoTbody);
    ...
    logMessage(`Error parsing Errors report HTML: ${outerError.message}`);
  }
...
module.exports = parseDeepErrorReport;
