...
const logFilePath = './log/parseSortSiteDeepUsabilityReport.log';
 ...
 */
function parseSEOExpando($expandoTbody) {
    const result = [];
    let currentDetails = [];
...
/**
 * Parses the full search report HTML into structured data.
 * @param {string} html
...
function parseDeepSEOReport(html) {
    ...
    try {
            ...
            $mainTbody.find('tr[id]').each(function () {
                try {
                    ...
                    if ($expandoTbody.length > 0) {
                        const expandoResult = parseSEOExpando($expandoTbody);
        ...
        logMessage(`Error parsing SEO report HTML: ${outerError.message}`);
    }
...
module.exports = parseDeepSEOReport;