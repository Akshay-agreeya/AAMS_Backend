...
const { getWebUrlsService, getAssessmentsService, getCategoryDataService, getUserWebUrlsService, insertCategoryAndDetailsService, updateCategoryAndDetailsService, deleteCategoryReport, getMobileScreenReportService, getMobileruleResultsService, getScreenshotService, getMobileRuleResultsRemediationService } = require("../services/reportsService");
...
exports.getMobileRuleResultsRemediationController = async(req,res,next) =>{
  const {mobile_screen_report_id} = req.params;
  const { mobile_rule_info_id, status } = req.query;

  try{
      const ruleResults = await getMobileRuleResultsRemediationService(mobile_screen_report_id, mobile_rule_info_id, status);
